    ...
    <div>
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          ...
          <!-- QQ 维修单下载 -->
          <a-icon  type="download"  @click="download(record.id,queryParams.repairStatus)" title="下 载"></a-icon>
        </template>
      </a-table>
    </div>
    <repair-add
...
import { newSpread, fixedForm, saveExcel } from "@/utils/spreadJSTwo";
moment.locale("zh-cn");
...
export default {
  ...
  methods: {
    ...
    download(id,params) {  
    console.log(params);
      this.$get("/cos/repair-info/repairInfoById", { repairId: id }).then(r => {  
        ...
        sheet.suspendPaint()
        sheet.setValue(1, 2, this.repairData.name)
        sheet.setValue(1, 4, this.repairData.code)
        sheet.setValue(2, 2, this.repairData.workName)
        sheet.setValue(2, 4, this.repairData.phone)
        sheet.setValue(3, 2, this.repairData.createDate)
        sheet.setValue(4, 2, `${this.repairData.address}+${this.repairData.floor}层+${this.repairData.number}`)
        sheet.setValue(5, 2, this.repairData.content)
        //  sheet.setValue(data[key].row, data[key].col, dataObject[key])
        // sheet.setValue(data[key].row, data[key].col, dataObject[key])
        spread = fixedForm(spread, 'textTable', { title: '维修信息单' })
        saveExcel(spread, `${this.repairData.code}号维修表.xlsx`)
